...
public class KingBooBrain {
    public static Brain<?> create(Brain<KingBooEntity> brain) {
        ...
        KingBooBrain.addFightTasks(brain);
    ...
    public static void updateActivities(KingBooEntity kingBoo) {
        ...
        kingBoo.setAttacking(brain.hasMemoryModule(MemoryModuleType.ATTACK_TARGET));
    }

    private static void addCoreActivities(Brain<KingBooEntity> brain) {
        brain.setTaskList(Activity.CORE, 10, ImmutableList.of(new LookAroundTask(45, 90), new WanderAroundTask()));
    }

    private static void addIdleActivities(Brain<KingBooEntity> brain) {
        brain.setTaskList(Activity.IDLE, ImmutableList.of(Pair.of(0, new UpdateAttackTargetTask<>(KingBooBrain::getAttackTarget)), Pair.of(1, new TimeLimitedTask<LivingEntity>(new FollowMobTask(EntityType.PLAYER, 6.0f), UniformIntProvider.create(30, 60))), Pair.of(2, new CompositeTask<>(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryModuleState.VALUE_ABSENT), ImmutableSet.of(), CompositeTask.Order.ORDERED, CompositeTask.RunMode.TRY_ALL, ImmutableList.of(Pair.of(new NoPenaltyStrollTask(1.0F), 2), Pair.of(new GoTowardsLookTarget(1.0F, 3), 3), Pair.of(new WaitTask(30, 60), 5))))));
    }

    private static void addFightTasks(Brain<KingBooEntity> brain) {
        brain.setTaskList(Activity.FIGHT, 0, ImmutableList.of(new ForgetAttackTargetTask<>(), new RangedApproachTask(1.4f), new MeleeAttackTask(20)), MemoryModuleType.ATTACK_TARGET);
    }

    private static Optional<? extends LivingEntity> getAttackTarget(KingBooEntity kingBooEntity) {
        return kingBooEntity.getBrain().getOptionalMemory(MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER);
    }
}
