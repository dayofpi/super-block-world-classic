...
public class ShoregrassBlock extends ToadstoolSoilBlock implements Fertilizable {
    public ShoregrassBlock(Settings settings) {
        super(settings);
    }
    ...
    public void spread(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (world.getLightLevel(pos.up()) >= 9) {
            for(int i = 0; i < 4; ++i) {
                ...
                if (world.getBlockState(blockPos).isOf(BlockRegistry.SHORESAND) && canSpread(state, world, blockPos)) {
