...
public class ModEntityRenderer<T extends MobEntity & IAnimatable> extends GeoEntityRenderer<T> {
    protected ModEntityRenderer(EntityRendererFactory.Context ctx, AnimatedGeoModel<T> modelProvider) {
    ...
    private int getOtherBlockLight(Entity entity, BlockPos pos) {
        if (entity.isOnFire()) {
            return 15;
        }
    ...
    public void render(T entity, float entityYaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider provider, int packedLightIn) {
        ...
        int r = this.getOtherBlockLight(holdingEntity, blockPos2);
