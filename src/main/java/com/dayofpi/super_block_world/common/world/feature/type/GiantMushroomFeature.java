package com.dayofpi.super_block_world.common.world.feature.type;
...
public class GiantMushroomFeature extends Feature<GiantMushroomFeatureConfig> {
        ...
        if (i >= world.getBottomY() + 1 && i + height + 1 < world.getTopY()) {
            ...
            if (!isSoil(blockState) && !blockState.isIn(BlockTags.MUSHROOM_GROW_BLOCK)) {
                ...
                for(int j = 0; j <= height; ++j) {
                    int maxCapRadius = this.getCapSize(config.capRadius.getMax(), j);

                    if (config.isFlat) {
                        maxCapRadius = this.getFlatCapSize(config.capRadius.getMax(), j);
                    }
    ...
    protected int getFlatCapSize(int capSize, int y) {
        return y <= 3 ? 0 : capSize;
    }

    protected int getCapSize(int capSize, int y) {
        int k = 0;
        if (y < -1 && y >= -1 - 3) {
            k = capSize;
        } else if (y == -1) {
