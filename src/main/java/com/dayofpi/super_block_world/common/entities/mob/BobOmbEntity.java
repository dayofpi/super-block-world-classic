...
public class BobOmbEntity extends AbstractEnemy implements Angerable {
    ...
    public static boolean canSpawn(EntityType<BobOmbEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return AbstractEnemy.isSpawnDark((ServerWorldAccess) world, pos, random);
    }
    ...
    public boolean damage(DamageSource source, float amount) {
        if (source.isFire() || source.getSource() instanceof FireballEntity || source.getSource() != null && source.getSource().isOnFire()) {
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        this.playSound(SoundInit.ENTITY_BOB_OMB_STEP, 0.6F, this.getSoundPitch());
    }
