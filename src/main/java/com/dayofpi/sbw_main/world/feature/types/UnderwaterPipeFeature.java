...
public class UnderwaterPipeFeature extends Feature<DefaultFeatureConfig> {
	...
	public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
		...
		OptionalInt optionalInt = getFloorHeight(world, blockPos, config);
		if (optionalInt.isPresent()) {
			blockPos = blockPos.withY(optionalInt.getAsInt());
		}

		if (world.getBlockState(blockPos).isOf(Blocks.WATER) && world.getBlockState(blockPos.down()).isSolidBlock(world, blockPos.down())) {
			BlockState blockState = ModBlocks.WARP_PIPE.getDefaultState().with(Properties.WATERLOGGED, true);
			BlockState blockState2 = ModBlocks.WARP_PIPE_BODY.getDefaultState().with(Properties.WATERLOGGED, true);
