...
public class SpreadableBlock extends ToadstoolSoilBlock implements Fertilizable {
    private final Block target;
    public SpreadableBlock(Block target, Settings settings) {
        super(settings);
        this.target = target;
    }
    ...
    public void spread(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (world.getLightLevel(pos.up()) >= 9) {
            for(int i = 0; i < 4; ++i) {
                ...
                if (world.getBlockState(blockPos).isOf(this.target) && canSpread(state, world, blockPos)) {
