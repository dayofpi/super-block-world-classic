...
import com.dayofpi.super_block_world.common.entities.hostile.ThwompEntity;
...
public class ThwompModel<T extends ThwompEntity> extends SinglePartEntityModel<T> {
    ...
    public void setAngles(T entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
        ...
        if (target != null) {
            ...
            this.rightEye.pivotX = MathHelper.sqrt((float)Math.abs(e)) * rotation * (float)Math.signum(e) - 1.5F;
            this.leftEye.pivotX = MathHelper.sqrt((float)Math.abs(e)) * rotation * (float)Math.signum(e) + 2.5F;

            if (this.rightEye.pivotX > -2.0F)
                this.rightEye.pivotX = -2.0F;
            if (this.leftEye.pivotX < 2.0F)
