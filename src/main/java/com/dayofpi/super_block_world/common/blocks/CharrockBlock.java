...
public class CharrockBlock extends Block {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        BlockPos blockPos = pos.add(random.nextInt(5) * multiplier, random.nextInt(20) + 5, random.nextInt(5) * multiplier);
        List<VolcanicDebrisEntity> debrisEntities = world.getEntitiesByClass(VolcanicDebrisEntity.class, Box.from(Vec3d.of(pos)).expand(16), EntityPredicates.VALID_LIVING_ENTITY);
        PlayerEntity closestPlayer = world.getClosestPlayer(pos.getX(), pos.getY(), pos.getZ(), 8D, EntityPredicates.VALID_LIVING_ENTITY);

        if (closestPlayer != null && debrisEntities.size() < 3 && world.getRegistryKey().equals(MushroomKingdom.WORLD_KEY) && random.nextInt(18) == 0 && blockPos.getY() < 0 && world.isSpaceEmpty(Box.from(Vec3d.ofCenter(blockPos)).expand(0, 6, 0).offset(0, -5, 0))) {
