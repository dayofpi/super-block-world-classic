...
import com.dayofpi.super_block_world.common.block.plant.*;
import com.dayofpi.super_block_world.common.world.feature.generator.AmanitaSaplingGenerator;
import com.dayofpi.super_block_world.common.world.feature.generator.DarkAmanitaSaplingGenerator;
...
public class PlantBlocks {
    ...
    public static final Block FUZZBALL = new Fuzzball(FabricBlockSettings.of(Material.PLANT, MapColor.BLACK).ticksRandomly().nonOpaque().noCollision().sounds(BlockSoundGroup.SLIME));
    public static final Block FUZZBUSH = new Fuzzbush(FabricBlockSettings.of(Material.PLANT, MapColor.BRIGHT_TEAL).ticksRandomly().nonOpaque().noCollision().sounds(BlockSoundGroup.AZALEA));
    ...
    public static final Block PIT_PLANT = new Muncher(FabricBlockSettings.of(Material.PLANT, MapColor.DARK_GREEN).nonOpaque().noCollision().sounds(BlockSoundGroup.GRASS));
    public static final Block MUNCHER = new Muncher(FabricBlockSettings.of(Material.PLANT, MapColor.BLACK).nonOpaque().noCollision().sounds(BlockSoundGroup.GRASS));
    public static final Block FROZEN_MUNCHER = new FrozenMuncher(FabricBlockSettings.of(Material.ICE, MapColor.LIGHT_BLUE_GRAY).nonOpaque().sounds(BlockSoundGroup.GLASS).slipperiness(0.98f).strength(0.5f).ticksRandomly());
    ...
    public static final Block FIRE_TULIP = new FireTulip(StatusEffects.FIRE_RESISTANCE, 6, FabricBlockSettings.of(Material.PLANT, MapColor.ORANGE).noCollision().sounds(BlockSoundGroup.GRASS).emissiveLighting(BlockInit::always));
