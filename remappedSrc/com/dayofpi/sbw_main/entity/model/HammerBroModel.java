...
public class HammerBroModel<T extends AbstractBro> extends BipedEntityModel<T> {
    ...
    public static TexturedModelData getTexturedModelData() {
        ...
        modelPartData.addChild("right_arm", ModelPartBuilder.create().uv(0, 34).mirrored().cuboid(-2F, 9.0F, -2.0F, 3.0F, 9.0F, 4.0F, new Dilation(0.0F)).mirrored(false), ModelTransform.pivot(-4.0f, 3.0f, 0.0f));
        modelPartData.addChild("left_arm", ModelPartBuilder.create().uv(0, 34).cuboid(-1F, 9.0F, -2.0F, 3.0F, 9.0F, 4.0F, new Dilation(0.0F)), ModelTransform.pivot(4.0f, 3.0f, 0.0f));
    ...
    public void setAngles(T entity, float f, float g, float h, float i, float j) {
        ...
        if (!entity.isAttacking()) {
            CrossbowPosing.meleeAttack(this.rightArm, this.leftArm, entity, this.handSwingProgress, h);
        }
    }
