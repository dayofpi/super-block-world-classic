package com.dayofpi.super_block_world.main.common.entity.type.bases;
...
public abstract class AbstractBuzzy extends CeilingEntity {
    ...
    public static boolean isSpawnBlockValid(WorldAccess world, BlockPos pos) {
        return world.getBlockState(pos).isOf(ModBlocks.VANILLATE) || world.getBlockState(pos).isOf(ModBlocks.TOPPED_VANILLATE) || world.getBlockState(pos).isOf(ModBlocks.FROSTY_VANILLATE);
    }
    ...
    public void doLandingEffects(float fallDistance, float damageMultiplier) {
        ...
        if (fallDamage > 0) {
            ...
            if (this.getType() == ModEntities.SPIKE_TOP)
                entities.forEach((entity) -> entity.damage(EntityDamageSource.thorns(this), fallDamage + 3));
            else
        ...
        if (fallDistance > 0.0F) {
            for (BlockPos blockPos : BlockPos.iterateOutwards(this.getBlockPos().down(), 1, 0, 1)) {
                if (world.getBlockState(blockPos).isIn(ModTags.BRICKS)) {
