...
public class BellTreeFeature extends Feature<DefaultFeatureConfig> {
    ...
    private void generateStem(WorldAccess world, BlockPos pos, int height, BlockPos.Mutable mutable) {
        for(int i = 0; i < height; ++i) {
            ...
            if (!world.getBlockState(mutable).isOpaqueFullCube(world, mutable)) {
                this.setBlockState(world, mutable, BlockInit.BELL_LOG.getDefaultState());
            }
        }
    ...
    protected void generateCap(WorldAccess world, BlockPos start, int y, BlockPos.Mutable mutable) {
        for(int i = 0; i < 4; ++i) {
            ...
            BlockState blockState = BlockInit.BELL_CAP.getDefaultState();

            if (inner.getY() % 2 == 0)
                blockState = BlockInit.DARKENED_BELL_CAP.getDefaultState();
            ...
            if (outer.getY() % 2 == 0)
                blockState = BlockInit.DARKENED_BELL_CAP.getDefaultState();
            else blockState = BlockInit.BELL_CAP.getDefaultState();

            BlockState blockState1 = BlockInit.DARKENED_BELL_CAP.getDefaultState();
